...
{
    int ret;
    //printk("MYCAT The msg_i2c_client->addr=0x%x i2c_client->timing=%d\n",i2c_client->addr,i2c_client->timing);
    i2c_client->addr|=I2C_ENEXT_FLAG;
	...
	//printk("MYCAT msg_i2c_read_interface OK\n");
    ...
    /*
        {
            ...
            SSL_PRINT("The CTP_ID=0x%x in %s(),L:%d\n",data,__func__,__LINE__);
        }

        if((i2c_smbus_read_i2c_block_data(i2c_client, 0x00, 1, &data)) < 0)
        {
            SSL_PRINT("I2C transfer error, func: %s\n", __func__);
            return -1;
        }
        SSL_PRINT("The CTP_ID=0x%x in %s(),L:%d\n",data,__func__,__LINE__);
    */
...
//////////////////////////////////////////////////////////////////////add by lan
	...
	//msg2033_i2c_read(reg_testval, 8);
